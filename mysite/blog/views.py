...
def get_blog_list_common_data(requst, blogs_all_list):
    # 每4篇进行分页
    ...
    # 获取当前页码前后更各两页的页码范围
    page_range = list(range(max(currentr_page_num - 2, 1), currentr_page_num)) + list(range(currentr_page_num,
                                                                                            min(currentr_page_num + 2,
                                                                                                paginator.num_pages)
                                                                                            + 1))
    # 加上省略页码标记
    if page_range[0] - 1 >= 2:
...
def blog_list(requst):
    ...
    context = get_blog_list_common_data(requst, blogs_all_list)
...
def blog_detail(requset, blog_pk):
    ...
    read_cookie_key = read_statistics_once_read(requset, blog)
    ...
    # 响应
    response = render(requset, 'blog/blog_detail.html', context)
    # 阅读cookie标记
